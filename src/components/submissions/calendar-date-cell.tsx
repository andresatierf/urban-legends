...
export function CalendarDateCell({
  ...
  const getCellStyles = () => {
    if (isOutsideTournament) {
      return "cursor-not-allowed border-2 border-dashed border-gray-200 bg-gray-50 text-gray-300";
    }

    if (isDisabled) {
      return "cursor-not-allowed border-2 border-dashed border-gray-200 bg-gray-50 text-gray-400 opacity-50";
    }

    // Base styles for clickable cells
    let styles =
    ...
    if (submission) {
      switch (submission.state) {
        ...
        case "pending":
          ...
          break;
        case "rejected":
          styles += " border-red-500 bg-red-50 text-red-800 hover:bg-red-100";
          break;
        case "deleted":
          styles +=
    ...
    // Today highlighting
    if (isToday) {
      styles += " ring-2 ring-blue-500 ring-offset-2";
    }
